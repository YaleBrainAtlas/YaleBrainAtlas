//var atlas_json = JSON.parse(fs.readFileSync('Atlas_Dict.json').toString());
...
OV.DetailsSidebarPanel = class extends OV.SidebarPanel
{
    ...
    {
        //var fs = require("fs");
        //var Atlas_Dict = JSON.parse('Atlas_Dict.json');
        var request = new XMLHttpRequest();
        //var request = new JSONHttpRequest();
        request.open("GET", "Atlas_Dict.json", false);
        request.send(null)
        var Atlas_Dict = JSON.parse(request.responseText);
        //var Atlas_Dict;
        //$.getJSON('Atlas_Dict.json', function(json){
            //Atlas_Dict = json;
        //});
        this.Clear ();
        ...
        this.AddProperty (table, new OV.Property (OV.PropertyType.Text, 'Parcel Name', element.name));
        this.AddProperty (table, new OV.Property (OV.PropertyType.Text, 'Description', Atlas_Dict.filter(({Code}) => Code === element.name).map(({Long_name}) => Long_name)));
        this.AddProperty (table, new OV.Property (OV.PropertyType.Text, 'Hemisphere', Atlas_Dict.filter(({Code}) => Code === element.name).map(({Hemisphere}) => Hemisphere)));
        this.AddProperty (table, new OV.Property (OV.PropertyType.Text, 'Lobe', Atlas_Dict.filter(({Code}) => Code === element.name).map(({Lobe}) => Lobe)));
        this.AddProperty (table, new OV.Property (OV.PropertyType.Text, 'Region', Atlas_Dict.filter(({Code}) => Code === element.name).map(({Region}) => Region)));
        this.AddProperty (table, new OV.Property (OV.PropertyType.Text, 'Gyrus', Atlas_Dict.filter(({Code}) => Code === element.name).map(({Gyrus}) => Gyrus)));
    ...
    AddMaterialProperties (material)
    {
        ...
        this.AddProperty (table, new OV.Property (OV.PropertyType.Text, 'Source', material.isDefault ? 'Default' : 'Default Atlas'));
